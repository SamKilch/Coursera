...
As there is quite a lot of data to process and we will build different models, we set up the machine to use multiple threads and to take advantage of parallel processing features to speed up the calculation times. Furthermore, we will use crossvalitation to increase the model accuracy.
